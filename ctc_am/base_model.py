...
def get_step_size(w, n_bits, lstm_flag, lstm_dim):
    s = 0.00001
    th  = 1e-6
    hist = [s]
    M = 2**(n_bits) - 1
...
class quantization_model(object):
    def __init__(self, n_bits_dict, pretrained_path):
        self.params = []
        self.grad_storage = []
        self.step_size = dict()
        self.n_bits = dict()
    ...
    def init_quantize(self):
        ...
        #create new variables for step_size and n_bits
        # for k in self.n_bits.keys():
        #     bit_name = k.split(':')[0]+'_bits'
        #     delta_name = k.split(':')[0]+'_delta'
        #     self.n_bits_tf[k] = tf.Variable(self.n_bits[k], trainable= False, name = bit_name, dtype = tf.int32)
        #     self.step_size_tf[k] = tf.Variable(self.step_size[k], trainable=False, name=delta_name, dtype = tf.float32)
